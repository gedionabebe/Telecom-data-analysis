...
def plotly_pie( df, column, limit=None):
...
def plotly_hist(df, column, color=['cornflowerblue']):
        fig = px.histogram(
                df,
                x=column,
                marginal='box',
...
def hist(df, column, color='cornflowerblue'):
...
def bar(df, x_col, y_col, title, xlabel, ylabel):
        plt.figure(figsize=(12, 7))
...
def box_multi(df, x_col, y_col, title):
        plt.figure(figsize=(12, 7))
